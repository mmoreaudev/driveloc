...
<?php if (!empty($dateError)): ?>
    <div class="alert alert-warning alert-dismissible fade show d-flex align-items-center gap-2" role="alert">
        ...
        <span><?= Security::e($dateError) ?></span>
...
<form method="GET" action="<?= APP_URL ?>/vehicles"
    ...
    <div class="row g-2 align-items-end mb-2">
        ...
        <div class="col-md-3">
            ...
            <input type="date" id="start_date" name="start_date"
                   ...
                   min="<?= date('Y-m-d') ?>"
                   value="<?= Security::e($raw['start_date'] ?? '') ?>">
        </div>
    ...
    <?php if ($hasFilters): ?>
        <div class="d-flex flex-wrap gap-1 mt-2 pt-2 border-top">
            ...
            ?>
                <span class="badge bg-warning text-dark"><?= Security::e($catName) ?></span>
            <?php endif; ?>
            <?php if ($hasDates): ?>
            ...
            <?php endif; ?>
            <?php if ($hasBrand): ?>
                <span class="badge bg-secondary"><?= Security::e($filters['brand']) ?></span>
            <?php endif; ?>
            <?php if ($hasPrice): ?>
                <span class="badge bg-secondary">≤ <?= Security::e($filters['max_price']) ?> €/j</span>
            <?php endif; ?>
        </div>
    <?php endif; ?>
                ...
                <div class="card h-100 border-0" style="box-shadow: 2px 2px 4px 1px #252525;">
                    ...
                    <div class="card-body d-flex flex-column">
                        ...
                        <span class="badge bg-warning text-dark mb-2" style="width:fit-content">
                            <?= Security::e($v['category_name']) ?>
                        </span>

                        <h2 class="h6 fw-bold mb-1"><?= Security::e($v['title']) ?></h2>
                        <p class="text-muted small mb-1">
                            <?= Security::e($v['brand']) ?> <?= Security::e($v['model']) ?>
                        </p>

                        <?php if ($hasDates): ?>
                            <!-- Période et estimation du total -->
...
(function () {
    ...
    // Couplage : end_date minimum = start_date + 1 jour
    startInput.addEventListener('change', function () {
        if (!this.value) return;
        const next = new Date(this.value);
        next.setDate(next.getDate() + 1);
        endInput.min = next.toISOString().split('T')[0];
        // Si end déjà rempli mais invalide, le vider
