        ...
        <tbody>
            <?php foreach ($users as $u): ?>
                <tr>
                    ...
                    <td><?= Security::e($u['firstname'] . ' ' . $u['lastname']) ?></td>
                    <td class="small"><?= Security::e($u['email']) ?></td>
                    <td>
                        <?php $roleBadge = ['admin' => 'bg-danger', 'owner' => 'bg-warning text-dark', 'client' => 'bg-primary']; ?>
                        <span class="badge <?= $roleBadge[$u['role']] ?? 'bg-secondary' ?>">
                            <?= Security::e($u['role']) ?>
                        </span>
                    </td>
                    ...
                    <td class="small text-muted"><?= Security::e(date('d/m/Y', strtotime($u['created_at']))) ?></td>
                    <td>
                        <?php if ($u['role'] !== 'admin'): ?>
                            <!-- Activer / Désactiver -->
                            <form method="POST"
                                  action="<?= APP_URL ?>/dashboard/admin/users/<?= $u['id'] ?>/toggle"
                                  class="d-inline">
                                <?= Security::csrfField() ?>
                                <button type="submit" class="btn btn-sm btn-outline-secondary me-1"
                                        title="<?= $u['status'] === 'active' ? 'Désactiver' : 'Activer' ?>">
                                    <i class="bi bi-<?= $u['status'] === 'active' ? 'toggle-on' : 'toggle-off' ?>"></i>
                                </button>
                            </form>
                            <!-- Supprimer -->
                            <form method="POST"
                                  action="<?= APP_URL ?>/dashboard/admin/users/<?= $u['id'] ?>/delete"
                                  class="d-inline"
                                  onsubmit="return confirm('Supprimer ce compte définitivement ?')">
                                <?= Security::csrfField() ?>
