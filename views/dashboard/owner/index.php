...
<?php if (empty($vehicles)): ?>
    <div class="text-center text-muted py-5">
        ...
        Vous n'avez encore publié aucun véhicule.
        <a href="<?= APP_URL ?>/vehicles/create" class="d-block mt-2 fw-semibold">Publier mon premier véhicule</a>
    </div>
<?php else: ?>
    <div class="table-responsive">
        <table class="table table-hover align-middle">
                ...
                <?php foreach ($vehicles as $v): ?>
                    <tr>
                        <td>
                            <?php if ($v['main_image']): ?>
                                <img src="<?= UPLOAD_URL . Security::e($v['main_image']) ?>"
                                     class="rounded object-fit-cover" style="width:70px;height:50px" alt="">
                            <?php else: ?>
                        ...
                        <td class="fw-bold"><?= number_format((float)$v['price_per_day'], 2, ',', ' ') ?> €</td>
                        <td>
                            <?php if ($v['status'] === 'active'): ?>
                                <span class="badge bg-success">Actif</span>
                            <?php else: ?>
                        ...
                        <td class="text-end">
                            ...
                            <!-- Activer / Désactiver -->
                            <form method="POST"
                                  action="<?= APP_URL ?>/vehicles/<?= $v['id'] ?>/toggle"
                                  class="d-inline" title="<?= $v['status'] === 'active' ? 'Désactiver' : 'Activer' ?>">
                                <?= Security::csrfField() ?>
                                <button type="submit"
                                        class="btn btn-sm <?= $v['status'] === 'active' ? 'btn-outline-warning' : 'btn-outline-success' ?> me-1">
                                    <i class="bi <?= $v['status'] === 'active' ? 'bi-pause-circle' : 'bi-play-circle' ?>"></i>
                                </button>
                            </form>
                            <!-- Supprimer -->
                            <form method="POST"
                                  action="<?= APP_URL ?>/vehicles/<?= $v['id'] ?>/delete"
                                  class="d-inline"
                                  onsubmit="return confirm('Supprimer ce véhicule définitivement ?')">
                                <?= Security::csrfField() ?>
                                <button type="submit" class="btn btn-outline-danger btn-sm" title="Supprimer">
